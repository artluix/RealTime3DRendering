...
namespace library
{
	std::map<library::RasterizerState, ComPtr<ID3D11RasterizerState>> RasterizerStateContainer::s_rasterizerStates;

	void RasterizerStateContainer::Initialize(ID3D11Device* const device)
	{
		assert(!!device);
	...
	void RasterizerStateContainer::Reset()
	{
		s_rasterizerStates.clear();
	}

	ID3D11RasterizerState* RasterizerStateContainer::GetRasterizerState(const RasterizerState rs)
	{
