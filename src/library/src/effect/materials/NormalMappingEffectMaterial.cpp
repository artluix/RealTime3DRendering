...
namespace library
{
	NormalMappingEffectMaterial::NormalMappingEffectMaterial(const Effect& effect)
	...
	NormalMappingEffectMaterial::~NormalMappingEffectMaterial() = default;

	void NormalMappingEffectMaterial::InitializeInternal()
	{
	...
	ComPtr<ID3D11Buffer> NormalMappingEffectMaterial::CreateVertexBuffer(ID3D11Device* const device, const Mesh& mesh) const
	{
		if (!mesh.HasVertices())
