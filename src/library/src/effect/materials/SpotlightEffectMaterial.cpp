...
namespace library
{
	...
	SpotlightEffectMaterial::~SpotlightEffectMaterial() = default;

	void SpotlightEffectMaterial::InitializeInternal()
	{
	...
	ComPtr<ID3D11Buffer> SpotlightEffectMaterial::CreateVertexBuffer(ID3D11Device* const device, const Mesh& mesh) const
	{
		if (!mesh.HasVertices())
