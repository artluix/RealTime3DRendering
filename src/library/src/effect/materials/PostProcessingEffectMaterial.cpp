...
namespace library
{
	PostProcessingEffectMaterial::PostProcessingEffectMaterial(const Effect& effect)
		: EffectMaterial(effect, "main11")
		, m_colorTexture(effect.GetVariable("ColorTexture"))
	{
	}

	PostProcessingEffectMaterial::~PostProcessingEffectMaterial() = default;

	void PostProcessingEffectMaterial::InitializeInternal()
	{
	...
	ComPtr<ID3D11Buffer> PostProcessingEffectMaterial::CreateVertexBuffer(ID3D11Device* const device, const Mesh& mesh) const
	{
		if (!mesh.HasVertices())
