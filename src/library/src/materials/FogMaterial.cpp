...
namespace library
{
	...
	void FogMaterial::InitializeInternal()
	{
		...
		CreateInputLayout(inputElementDescriptions, "fogEnabled");
		CreateInputLayout(inputElementDescriptions, "fogDisabled");
	}

	ComPtr<ID3D11Buffer> FogMaterial::CreateVertexBuffer(ID3D11Device* const device, const Mesh& mesh) const
	{
		if (!mesh.HasVertices())
