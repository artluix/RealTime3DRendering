...
namespace library
{
	EnvironmentMappingMaterial::EnvironmentMappingMaterial(const Effect& effect)
	...
	ComPtr<ID3D11Buffer> EnvironmentMappingMaterial::CreateVertexBuffer(ID3D11Device* const device, const Mesh& mesh) const
	{
		if (!mesh.HasVertices())
