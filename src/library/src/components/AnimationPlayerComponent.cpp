...
namespace library
{
	AnimationPlayerComponent::AnimationPlayerComponent(
		const Model& model, const bool enableInterpolation /*= true*/
	)
		: m_model(model)
	...
	void AnimationPlayerComponent::Update(const Time& time)
	{
		...
		m_time += std::chrono::duration_cast<Duration>(time.elapsed.GetDuration()) *
			m_animation->GetTicksPerSecond();
	...
	)
	{
		...
		const auto& toParentTransform = !!bone ?
			m_animation->GetTransform(*bone, timePoint) :
	...
	)
	{
		...
		const auto& toParentTransform = !!bone ?
			m_animation->GetTransform(*bone, keyframe) :
			sceneNode.GetTransform();
	...
	)
	{
		...
		const auto& toParentTransform = !!bone ?
			m_animation->GetInterpolatedTransform(*bone, timePoint) :
			sceneNode.GetTransform();
