...
void DemoApplication::Initialize()
{
	...
	auto multipleLights = std::make_shared<MultipleLightsDemo>();
	multipleLights->SetCamera(*camera);
	multipleLights->SetKeyboard(*m_keyboard);
		...
		if (m_keyboard->WasKeyPressed(Key::Left_Ctrl))
		{
			...
			for (auto component : m_components)
			{
				if (auto drawableComponent = component->As<DrawableComponent>())
				{
					...
					{
						if (auto lightMaterial = material->As<LightsMaterial>())
						{
